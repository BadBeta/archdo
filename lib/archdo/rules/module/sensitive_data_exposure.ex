defmodule Archdo.Rules.Module.SensitiveDataExposure do
  ...
  @sensitive_fields [
    :password, :password_hash, :password_digest, :hashed_password, :encrypted_password,
    :secret, :secret_key, :secret_key_base, :api_key, :api_secret,
    :token, :auth_token, :access_token, :refresh_token, :session_token, :reset_token,
    :private_key, :signing_key, :encryption_key,
    :credit_card, :card_number, :cvv, :ssn, :social_security,
    :otp_secret, :totp_secret, :recovery_codes
  ]

  # String patterns that indicate hardcoded secrets
  @secret_prefixes ["sk_live_", "sk_test_", "pk_live_", "pk_test_",
      ...
      case {sensitive, has_overbroad_jason_derive?(derives)} do
        {[_ | _], true} ->
          ...
          [Diagnostic.warning("6.56",
            ...
            message:
              "`@derive Jason.Encoder` encodes ALL fields — including #{names}",
            why:
  ...
  defp find_sensitive_in_logger(file, ast) do
    Enum.map(AST.find_all(ast, fn
      ...
      Diagnostic.info("6.56",
        ...
        message:
          "Logger message interpolates a variable with a sensitive name",
        why:
  ...
  defp build_secret_diagnostic(file, line) do
    Diagnostic.warning("6.56",
      ...
      message:
        "A string that looks like a secret or API key is hardcoded in source code",
      why:
  ...
  defp find_sensitive_in_error_message(file, ast) do
    Enum.map(AST.find_all(ast, fn
      ...
      Diagnostic.info("6.56",
        ...
        message:
          "raise/error message interpolates a potentially sensitive variable",
        why:
  ...
  defp sensitive_field?(field) when is_atom(field) do
    field in @sensitive_fields or
      String.contains?(Atom.to_string(field), ["password", "secret", "token", "api_key", "private_key"])
  end
  ...
  defp sensitive_attr_name?(name) do
    ...
    Enum.any?(["secret", "password", "api_key", "token", "private_key", "signing_key"],
  ...
  defp sensitive_variable_name?(var) do
    ...
    Enum.any?(["password", "secret", "credential", "token", "api_key", "private_key"],
