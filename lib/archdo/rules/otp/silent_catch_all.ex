    ...
    case AST.genserver_module?(ast) do
      ...
      |> Enum.map(fn {meta, _args, _body} ->
        Diagnostic.info("5.14",
          ...
          alternatives: [
            Fix.new(
              summary: "Delete the catch-all and let GenServer's default log unexpected messages",
              detail:
                "Removing the clause is the safest option. Elixir 1.15+ logs unhandled handle_info messages " <>
                  ...
                  "rule is asking for.",
              applies_when: "You're on Elixir 1.15+ (the default behaviour logs unhandled messages)."
            ),
            Fix.new(
