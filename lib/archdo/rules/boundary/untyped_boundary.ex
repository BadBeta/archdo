defmodule Archdo.Rules.Boundary.UntypedBoundary do
  ...
  defp untyped_diag_for_spec(_node, _file), do: []

  # §§ elixir-implementing: §2.1 — boolean → multi-clause head
  defp diag_if_untyped(false, _spec_args, _file, _meta), do: []

  defp diag_if_untyped(true, spec_args, file, meta) do
    fn_name = extract_spec_fn_name(spec_args)
    [build_untyped_diagnostic(fn_name, file, meta)]
  end

  defp build_untyped_diagnostic(fn_name, file, meta) do
